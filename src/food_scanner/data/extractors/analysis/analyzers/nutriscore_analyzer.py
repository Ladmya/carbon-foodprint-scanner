"""
src/food_scanner/data/extractors/analysis/analyzers/nutriscore_analyzer.py
...
class NutriscoreAnalyzer(BaseFieldAnalyzer):
    ...
    RESPONSIBILITY: Multi-sources extraction  + grade ↔ score cross validation
    """
    
    def analyze_nutriscore_from_extracted_products(self, products: List[Dict[str, Any]]) -> FieldAnalysisResult:
        """
        Analyze nutriscore extraction from extracted products
        
        Args:
            products: Output from ProductExtractor.run_complete_extraction()
        """
        ...
        if self.field_name == "nutriscore_grade":
            ...
            result.value_distribution = self._create_nutriscore_score_bins(score_distribution)
        ...
        if self.field_name == "nutriscore_grade":
            result.pattern_analysis.update({
                ...
                "grade_completeness_by_source": self._analyze_nutriscore_grade_sources(products)
            })
        elif self.field_name == "nutriscore_score":
            result.pattern_analysis.update({
                "score_statistics": self._calculate_score_statistics(score_distribution),
                "score_grade_consistency": self._analyze_nutriscore_score_grade_consistency(products)
            })
    ...
    def _analyze_nutriscore_grade(
        ...
        if grade:
            ...
            if score is not None:
                expected_grade = self._convert_nutriscore_score_to_grade(score)
            ...
            score = self._extract_nutriscore_score(product)
            if score is not None:
                calculated_grade = self._convert_nutriscore_score_to_grade(score)
    ...
    def _convert_nutriscore_score_to_grade(self, score: float) -> str:
    ...
    def _create_nutriscore_score_bins(self, score_distribution: List[float]) -> Dict[str, int]:
    ...
    def _calculate_nutriscore_score_statistics(self, score_distribution: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "standard_deviation": self._calculate_standard_deviation(score_distribution),
            "quartiles": self._calculate_quartiles(score_distribution)
        }    
    
    def _analyze_nutriscore_grade_sources(self, products: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze the completeness by source for grades"""
    ...
    def _analyze_nutriscore_score_grade_consistency(self, products: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze the consistency between scores and grades"""
        consistency_stats = {
            "total_with_both": 0,
            "consistent": 0,
            "inconsistent": 0,
    ...
    def _generate_nutriscore_extraction_recommendations(
        ...
        if calculated_from_score > 0:
            result.transformation_recommendations.append(
                f"Grade calculated from score used for {calculated_from_score} products"
            )
        ...
        if len(extraction_sources) > 1:
            ...
            result.transformation_recommendations.append(
                f"Detected multiple sources: {sources_list}"
            )
        
        # Recommendations on cross validation
        if cross_validation_issues:
            result.quality_improvement_suggestions.append(
                f"Inconsistencies grade/score detected: {len(cross_validation_issues)} cases"
            )
            result.transformation_recommendations.append(
                "Implement cross-validation grade ↔ score to detect errors"
            )
        ...
        if result.missing_count > 0:
            if self.field_name == "nutriscore_grade":
                result.quality_improvement_suggestions.append(
                    f"Missing nutriscore grades: {result.missing_count} products"
                )
            else:
                result.quality_improvement_suggestions.append(
                    f"Missing nutriscore scores: {result.missing_count} products"
                )
        ...
        if self.field_name == "nutriscore_score":
            ...
            if score_stats.get("count", 0) > 0:
                ...
                result.transformation_recommendations.append(
                    f"Average score: {avg_score:.1f} (distribution: {result.value_distribution})"
                )
        
        # Recommendations general quality
        if result.validity_rate < 90 and result.present_count > 0:
            result.quality_improvement_suggestions.append(
                f"Nutriscore quality under optimal: {result.validity_rate:.1f}% validity"
            )